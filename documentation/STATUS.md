...
| I2 resumable terminal setup and HTTPS | Built | `ops/installer/bootstrap`, `ops/installer/navishai` (`setup`) | Native: consent, answer files, occupied ports, same-release resume, HTTPS-only readiness with no HTTP fallback, publish interruption, image-load failure, validated explicit public IPv4 Caddy binding that leaves listeners on other interfaces untouched, and permanent `www`-to-apex HTTPS redirect. Disposable: local-CA install only. Public ACME, real reboot, SSH loss, and a clean supported-host run of the exact candidate remain external. |
| I3 Owner handoff and checklist | Built | `FirstOwnerBootstrap`, `WorkspaceSetupChecklist`, `navishai configure {system-mail,memory,scanner}` | Expiry and once-only claim, terminal-only token reveal, expired-token renewal before any Owner exists, and skipped/configured/tested/blocked states are tested; configured is never shown as tested. An Owner-triggered synthetic scanner check (clean fixture plus EICAR test signature, recorded as an append-only `attachment_scanner` operational check bound to the scanner configuration digest) distinguishes configured, reachable-and-classifying, and blocked; real-malware coverage stays external. An Owner-triggered synthetic memory check writes a uniquely identified record through the internal contract, proves scoped indexing and retrieval, proves another Workspace cannot retrieve it, then tombstones and removes it. The append-only `memory_verification` check is bound to a non-secret configuration digest; pending and failed results cannot be verified; changed configuration cannot inherit an earlier pass; NavishAI never falls back to a managed memory host. Live Supermemory proof remains external. |
| I4 day-two commands | Partial | `ops/installer/navishai` (`status`, `doctor`, `backup`, `restore`, `upgrade`) | Fixed-path backup/restore and exact release-identity restore are native and disposable tested. Production `upgrade` resolves a commit from the configured deployment-owned source checkout's `origin/main` (or configured safe ref), builds an immutable detached-worktree candidate, creates and verifies a unique pre-upgrade backup, and reports the recovery path after internal and public health checks; it accepts no operator-supplied release path, candidate, or backup; an explicit target is limited to a validated origin ref. It accepts an identical image archive, or a changed application image when saved-image identity, topology, infrastructure pins, PostgreSQL major, and migrate-status checks pass; unsupported and indeterminate targets are still rejected before writers stop. Native fake-Docker tests cover success, rollback, unrecoverable restore, resume, schema/topology/runner rejection, and the retained opaque-archive guard. Live digest proof on a Docker host is outstanding ([APPLICATION_IMAGE_UPGRADE_ACCEPTANCE.md](./APPLICATION_IMAGE_UPGRADE_ACCEPTANCE.md)). `doctor` is read-only, lists every finding with a recovery instruction, and offers `--json`. |
| I5 fresh-host acceptance | External | [INSTALLER_ACCEPTANCE_EVIDENCE.md](./INSTALLER_ACCEPTANCE_EVIDENCE.md) | Requires a clean supported Ubuntu 24.04/26.04 or Debian 12 host, the exact candidate, public DNS/ACME, external ingress isolation checks, and an admin who did not build the installer. The 12 September 2026 cloud-agent host recorded these as omitted; nothing here is inferred from mocked tests. |
...
| Self-hosted Supermemory integration with tenant isolation | Done | `SupermemoryEngine`, `MemoryEngine`, `MemoryIndexer`, `MemoryIndexEntry`, `MemoryVerificationCheck` | Managed host rejected; stable Memory key is the document identity. Owner/Admin synthetic round trip is fixture-tested; live engine proof is external. |
...
On `cursor/scorecard-preview-efe6`, cherry-picked onto E1 and E2. Publish and rollback lock the scorecard and version before selecting the latest `generated_at` backtest. Preview generation takes the same version lock. A later-created row with an earlier `generated_at` is not treated as the inspected preview. Duplicate concurrent publishes stay idempotent (one current version, one `scorecard.published` audit). Focused checks on C3: scorecard service/controller/proposal plus the publish race, 24 runs, 181 assertions. RuboCop clean on touched Ruby files. Isolation, Docker, and full `bin/ci` omitted on this host.
...
On `cursor/phase-handoff-efe6` at `fdd3145`, Linux 6.12.94+ amd64, Ruby 4.0.6, Go 1.27.1, PostgreSQL 15.19 with pgvector 0.8.6 (host deviation vs pinned 16), Chrome 148.0.7778.96 with matching ChromeDriver, LibreOffice 24.2.7. Command: `CHROME_BIN=/usr/local/bin/google-chrome CHROMEDRIVER_BIN=/opt/chromedriver/148.0.7778.96/chromedriver bin/ci` with no controlling TTY (`setsid`). All declared steps passed in 4m38.72s, including `NAVISHAI_REQUIRE_ISOLATION_TESTS=1` Go tests (Landlock syscall ABI 6; `/sys/kernel/security/landlock` is absent). Rails: 1162 runs, 8494 assertions. System: 87 runs, 1414 assertions. Fixes that unblocked this run: bind account-work-queue identifiers for Brakeman; truncate audit events together with due notices; raise isolated LibreOffice process and file limits from 64/128 to 512. ChromeDriver 148.0.7778.96 was installed without a recorded checksum (`sha256 a27b110f1c36c07bae65ef4469c424f291ab31d48d2d040513ab17be9d0e68b0`). Docker Engine is not installed. A TTY-backed `bin/ci` hang is not a product failure: installer setup reads `/dev/tty` for confirmation. No merge, release, or deploy.
