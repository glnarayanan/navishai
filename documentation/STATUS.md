...
2. **Native runtime search.** **No-go confirmed 9 September 2026.** The smallest approved candidate is Codex `exec --json`, supported by this repository from `0.149.0` with no upper version cap. Its maintained upstream stream has `web_search` items with query/action data, but no typed results, canonical result URL, bounded source excerpt, publication date, result retrieval time, or result-level terminal status. The stream has no independent protocol version. Upstream app-server result JSON is optional and opaque, and `exec --json` does not emit it. The installed CLI was absent; no opt-in smoke ran because it can consume subscription usage. `web_search="disabled"` remains fixed, and parser tests reject a native-search item as prohibited tool use. Query events, action URLs, generated prose, inferred URLs, and opaque output cannot become citations or satisfy grounding. A future adapter needs a versioned typed result contract plus an approved egress profile. Parallel remains unimplemented.
