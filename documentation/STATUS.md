...
The installer has native fake-Docker coverage for bundle validation, locks, answer files, consent-gated setup, safe same-release resume, no-token terminal handoff to first-Owner setup, protected system-SMTP configuration, backup/restore helpers, and same-major changed-image upgrade success and failure phases. Changed images now load and start from an immutable target release while `current` remains on the old release until migrations and readiness finish; any failure stops writers and requires the verified restore path. Native tests do not prove application-image or schema compatibility. `navishai configure system-mail` replaces only the five system-mail fields from a protected password-file reference and restarts web/jobs; it does not send mail or configure shared-inbox SMTP. Local SMTP tests prove required trusted STARTTLS before authentication and reject a server without STARTTLS before `AUTH`. Disposable Docker evidence covers local-CA first Owner and browser setup, runner ownership, signed callback and artifact flow, pending-memory service suppression, isolated restore of an attachment, memory sentinel, and vault readability metadata, including the managed backup-metadata restore path. The restore proof does not validate provider credentials or ClamAV. Public ACME, release signing, a disposable changed-image/schema recovery rehearsal, and a safe pinned-Supermemory first-boot provisioning path remain incomplete. The pinned opaque `server-v0.0.8` binary has no documented noninteractive bearer-key bootstrap or key export interface; current upstream guidance also requires an external provider credential for its TTY first boot. The only evidenced safe path is an isolated human capture followed by protected key entry. The explicit `reveal-owner-token --confirm-reveal` command warns about terminal recording. Production cache, queue, and cable databases use Ruby schemas while primary uses SQL.
