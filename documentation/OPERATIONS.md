...
`navishai restore` selects the retained release whose `SOURCE_COMMIT` matches the verified backup manifest and refuses if it is absent. `navishai upgrade` accepts an identical image archive, or a changed **application** image when the candidate Docker-save manifest proves runner and Supermemory images are unchanged, Compose topology and infrastructure references match, PostgreSQL major stays the same, and the target reports no pending migrations. The installer loads the new application image before it stops writers, promotes only after that check, and restores the previous application image if readiness fails. Opaque archives, runner or memory image changes, schema-changing targets, and PostgreSQL major mismatches stay rejected. A failed same-archive migration still requires restore from the verified backup. This does not prove general schema-change compatibility.
