...
`navishai restore` selects the retained release whose `SOURCE_COMMIT` matches the verified backup manifest and refuses if it is absent. Production upgrades are one command: `navishai upgrade`, with `navishai upgrade --target <ref>` as the bounded explicit form. The deployment owner configures a root-owned `NAVISHAI_UPDATE_SOURCE_PATH` for the trusted Git checkout and may set `NAVISHAI_UPDATE_SOURCE_REF` (default `main`). The command fetches only that checkout's `origin`, validates and resolves one immutable commit, creates a detached worktree, and builds its candidate through `script/candidate_bundle`. It then preserves every existing topology, infrastructure, PostgreSQL-major, runner, memory, and schema refusal, creates and verifies a uniquely named managed pre-upgrade backup, and reports the selected source commit, installed release identity, and exact recovery command only after internal startup and public HTTPS `/up` verification pass. Production callers never supply a release path, candidate, or backup path. A changed **application** image is still allowed only when the candidate Docker-save manifest proves runner and Supermemory images are unchanged; the installer loads it before stopping writers and restores the previous application image if readiness fails. Opaque archives, runner or memory image changes, schema-changing targets, and PostgreSQL major mismatches stay rejected.
