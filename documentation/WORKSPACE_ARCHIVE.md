...
The M6 integrated proof verifies that one GET-only historical Intercom record, its bounded preservation report, exact counts, and report digest survive the archive round trip. It also verifies that the imported available Memory record reconstructs from PostgreSQL without copying an engine-private document ID.
