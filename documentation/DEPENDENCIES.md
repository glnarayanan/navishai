...
| Caddy | `caddy:2.10.2-alpine` pinned by digest `sha256:4c6e91c6ed0e2fa03efd5b44747b625fec79bc9cd06ac5235a779726618e530d` in `ops/installer/compose.yaml` | Owner-approved on 10 September 2026 for guided Compose HTTPS. It is the sole public app ingress, retains ACME account and certificate state in named volumes, and has its admin API disabled. It keeps only `NET_BIND_SERVICE` so it can bind 80 and 443. The digest is the multi-architecture OCI index for that tag as served by the Docker Hub registry API on 11 September 2026; it has not yet been pulled on a Docker host, and public certificate issuance and renewal proof remain outstanding before release. |
