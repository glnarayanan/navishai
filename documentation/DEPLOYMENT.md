...
Supermemory needs its first-boot local model setup. Start it, complete that setup according to its local prompt, then place its generated key in `NAVISHAI_SUPERMEMORY_API_KEY` and recreate `web` and `jobs`. A placeholder value may be used for the first Supermemory boot. Set `NAVISHAI_MEMORY_PENDING=1` while that key is unavailable: the installer leaves Supermemory stopped and the authenticated checklist marks Memory as deferred. Remove it only after configuring the key and restarting the Rails services. The Lite build has a 10,000-document licence cap.
