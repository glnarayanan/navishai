...
- **Knowledge improvement queue:** one Workspace-scoped, read-only list of knowledge sources that are stale, deleted, retired after confirmed absences, or tied to a failed sync pass. A source leaves the queue when its current version is no longer stale; prior versions remain as follow-up evidence. It does not mutate knowledge or send messages.
