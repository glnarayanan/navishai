...
The opt-in upstream contract smoke uses the installed subscription and is off by default: `NAVISHAI_CODEX_LIVE_SMOKE=1 CODEX_HOME=/path/to/codex-home go test ./runner/internal/adapters/codex -run LiveSubscriptionSmoke`. It may consume subscription usage. The smoke checks the real CLI stream but does not replace normal supervisor boundary tests. The current production supervisor still denies all network syscalls; a provider-neutral egress boundary must land before live runtime execution can leave the runner.
