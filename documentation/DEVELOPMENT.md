...
The signed `POST /v2/runtimes/detect` runner endpoint reports only executables registered by installed adapters. It resolves each path, runs a bounded version probe, compares semantic versions with the adapter's maintained range, and returns declared capabilities, the deployment-selected effective model, a non-secret configuration fingerprint, and non-secret account details. The fingerprint is an HMAC-SHA256 value keyed by the runner shared secret over the effective adapter and egress configuration, including environment values; those values never leave the runner. The runner retains `POST /v1/runtimes/detect` with its legacy response shape so an older control plane remains compatible during a runner-first rolling deployment. Owners and Admins use **Providers** to refresh those facts, explicitly test the current configuration, and approve routing profiles, workspace roles, tools, data classes, and time, step, tool-call, input-unit, and output-unit caps. The signed `POST /v1/runtimes/test` endpoint is available only to adapters that declare the `runtime_test` capability. It runs a fixed sentinel through the configured adapter and production supervisor with an empty isolated working directory, no supplied tools, files, Memory, public-web query, or Workspace content, and tight step, unit, and time budgets. Rails stores only pass or bounded failure state, the tested fingerprint and time, and observed unit counts. It never stores the prompt or runtime output. PostgreSQL refuses approval without a passing test for the current fingerprint and revokes approval and test evidence when material detection facts change or an installation disappears. Subscription credentials stay on the runner and never enter either response or a business row.
