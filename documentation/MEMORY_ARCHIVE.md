...
Import is a restore operation. The target Workspace key and referenced Account, Contact, Case, Crew, Agent, User, Membership, and artifact rows must already match the archive. The target must contain no Memory records. NavishAI rejects another Workspace's archive, unknown formats, broken supersession links, missing references, malformed records, and files over 20 MiB. The whole import commits or rolls back as one transaction.
