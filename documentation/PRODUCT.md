...
- Knowledge sources from maintained text, URL snapshots, uploaded documents (text, Markdown, HTML, PDF, DOCX, legacy DOC, ZIP bundles), and Intercom Help Center.
...
3. Store interventions separately from crew narratives with the states proposed, approved, completed, abandoned, and reviewed. A Manager-or-higher human approves or abandons, may reassign a proposed or approved intervention to another writable human, and may change the follow-up date with a recorded reason. Completion is a human action by the current accountable person and never sends customer communication; review freezes before-and-after facts and reports association, not cause. Due and overdue internal notices use existing notification infrastructure, dedupe by intervention, recipient, due-state, and target date, and never go to customers or former assignees.
...
- Personal AI accounts belong to the connecting user within a Workspace. The web interface initiates provider-supported authentication on the deployed runner; no local companion executes work or exports desktop login credentials. Admission freezes account ownership and configuration, and a personal-account selection cannot fall back to another person’s credentials. Subscription credentials stay on the customer-controlled runner; NavishAI detects and invokes an authenticated CLI but never imports, copies, displays, or stores its login token. API keys cross Rails only in the synchronous signed configure request and live in the encrypted Workspace-scoped runner vault; Workspace deletion purges them first.
...
- Public-web search is a typed `web_search` tool behind a provider-neutral registry with SearXNG as the self-hosted default and hosted adapters (Exa, Tavily) as options that keep their API key on the runner. A runtime's native search is a further option only when the run's egress profile permits it and results are auditable and structured. Owners and Admins select authorised search providers; other users only use configured providers. Native search has an explicit Admin disclosure that the runtime composes and transmits queries, and is unavailable unless its structured evidence contract is proven. Stored queries are minimised and redacted; results are normalised with URL, excerpt, publication and retrieval dates, citation, policy decision, and cost; evidence links are HTTPS only.
...
- **Connector credentials.** Owners and Admins enable Intercom and Notion per Workspace and configure Workspace service credentials. Users may authorise their own provider accounts only while that connector is enabled. Workspace and personal credentials are distinct, encrypted, excluded from archives, and checked again before use. Personal resources are not automatically published into shared knowledge. Provider-supported OAuth is the initial personal connection path; MCP requires a separately bounded read-only tool contract.
- **Knowledge mappings.** Preserve the originating remote Workspace and article identity. Knowledge may apply to multiple products and Intercom connections inside the same NavishAI Workspace. Connection defaults apply until a knowledge-managing human records an explicit override; sync preserves overrides. Case retrieval and evidence admission enforce applicability.
- **Knowledge.** Approved sources are maintained text, HTTPS URL snapshots, uploaded documents (text, Markdown, HTML, PDF, DOCX, legacy DOC, and ZIP bundles of them, each scanned and reduced to a plain-text snapshot with the original retained), and Intercom Help Center articles. Track source, version, retrieval or sync time, expiry, and deletion; warn on stale or deleted material; current authoritative content beats memory. Help Center and Admin-configured Notion roots synchronise through bounded complete passes; successful observations refresh current evidence without rewriting historical versions. See STATUS.md for verification and live-credential boundaries.
