...
- a reference ClamAV attachment-scanner adapter and a daily scheduled Account-health pass;
...
The 6 September 2026 checkpoint ran from main `5e35f5e3d3d536f0006bdfc0af47474f0dbcf459` plus the scanner, schedule, and supervisor-test changes recorded in the roadmap, on a fresh Linux 6.18 x86-64 host with Ruby 4.0.6 built from the `ruby_4_0` branch at 4.0.6 patch level 0, Go 1.27.0, PostgreSQL 16.15, and pgvector 0.8.6 built from the pinned source revision. In that environment:

- the M6 integrated proof passed 3 tests with 97 assertions;
- the full Rails suite passed 864 tests with 6,469 assertions and no skips;
- RuboCop inspected 534 files with no offences, Brakeman reported no warnings, and the gem and Importmap audits passed;
...
- the seed replant and the 84-component SBOM check passed.

Two checks could not run on that host and are recorded as omissions, not passes. The browser suite needs a Chrome for Testing build with a matching driver, which the host's network policy did not allow; its last recorded green run is PR #82's 60 system tests with 957 assertions on 28 August 2026, before the landing-page changes in PRs #85–#88. The supervisor's process-boundary tests skip on that host because its kernel has no Landlock; with `NAVISHAI_REQUIRE_ISOLATION_TESTS=1`, as `bin/ci` sets it, they fail rather than skip, so the runner isolation evidence still comes from the 27 August native Linux run below. PostgreSQL 16 was used because PostgreSQL 15 packages were unreachable; the pinned target remains PostgreSQL 15.
...
- The browser suite has not been rerun since the landing-page changes in PRs #85–#88. Run `bin/rails test:system` on a host that can fetch Chrome for Testing before the next checkpoint.
...
- Public-web search has one approved provider, SearXNG. Knowledge uploads accept plain text only, and Intercom Help Center knowledge is a manual snapshot rather than a synchronised source. Roadmap milestone M7 covers these.

### Credential boundaries

- Live OpenID Connect, SMTP, Intercom, SearXNG, ClamAV, object-store, provider API-key, and subscription-runtime smoke tests need deployment-owned endpoints or credentials. The default suites use protocol fixtures and must not consume customer accounts.
