...
| M6 | Phase-completion proof | M5 | Complete | Deterministic integrated journeys at `5e74ed6cce04cb730c376b0328c7a814812feae1` and strengthened proof at `2f1f876884866ac0046a81b5795e7570669ee96d`, with parallel attachment-storage isolation at `1a890c664babf93766f6c88ecaf9c0ed9d7419ac`: three phase journeys passed with 90 assertions; 172 focused adversarial tests passed with 1,530 assertions; 60 desktop, 320-pixel mobile, keyboard, failure, degraded, and recovery browser tests passed with 957 assertions; and `bin/ci` passed 509-file Ruby style, Go style, dependency audits, Brakeman with no warnings, 718 Rails tests with 5,104 assertions, those 60 browser tests, Go vet and tests, three Linux builds, the Rails-to-runner contract, seed replant, and the 84-component SBOM. A native PostgreSQL backup with SHA-256 `83ad5f35cc32d063518a8773718b0ac939d70a912e13f49bb5c39cdc57ac76d1` restored into an isolated database with 105 public tables, 61 migrations, and pgvector 0.8.6. Inspected final captures showed no visible overflow or clipping, and an Impeccable layout scan found no defects; Ui.sh was unavailable in this thread. Docker and Podman were unavailable, so live image, Compose backup and restore, Compose preflight, and live pgvector 0.8.1 upgrade checks remain host work. The separate registered performance worker retains the three-baseline comparison. |
...
- Build a role-gated operator view over existing connector, webhook, queue, execution, outbound-delivery, memory-index, retention, export/import, and audit state.
- Show at least connector readiness and freshness, reconciliation cursor age and drift, failed or replayed inbound deliveries, queue lag, unconfirmed run admission, retryable and terminal runs, unknown sends, indexing backlog or failure, latest archive verification, latest backup or restore rehearsal, and upgrade-preflight state.
...
- Failure-injection fixtures cover stale connectors, queue lag, replay, runner admission ambiguity, definite retry, unknown send, indexing outage, failed operational check, and recovery idempotency.
