...
- Add at least one hosted search adapter beside SearXNG behind the existing provider-neutral `web_search` contract in the Go runner. Prefer a provider whose API returns structured results with URL, title, excerpt, and publication date, needs only one API key held on the runner, and reports cost units.
