...
| M6 | Phase-completion proof | M5 | Complete | Merged 28 August 2026 in PR #82 (`2e6dd3f`). Re-proven from code on 6 September 2026 at main `5e35f5e3d3d536f0006bdfc0af47474f0dbcf459` on a fresh Linux host: `test/integration/phase_completion_proof_test.rb` passed 3 tests with 97 assertions through ordered `ExecutionLedger` terminal events, `CrewArtifactPublisher`, `ResolutionContractEvaluator`, `CrewWork` review transitions, human send, explanation, dossier, intervention, reliability recovery, backfill, archive round trip with Memory reconstruction, and governed canary and rollback. The same run passed 864 Rails tests with 6,469 assertions, RuboCop on 534 files, Brakeman, gem and Importmap audits, Go vet and the 17-package Go suite, the runner contract, seeds, and the 84-component SBOM check. A later run on the same host with a matching ChromeDriver passed 67 browser tests with 1,159 assertions; the kernel-isolation suite remains a host omission whose last green evidence is recorded in `RELEASE_CANDIDATE.md`. Ui.sh was not invoked for this source proof. |
| M7 | Knowledge and research intake | M6 | In progress | M7.1 document formats and M7.3 hosted search adapters landed on 6 September 2026 on `claude/docs-build-contracts-review-nx2rly`: `.txt`, `.md`, `.html`, and `.pdf` uploads plus `.zip` bundles through `KnowledgeDocumentExtractor` and the standard-library `KnowledgeZipBundle`, with the owner-approved `pdf-reader` gem; Exa and Tavily runner adapters beside SearXNG with tests for key custody, bounded bodies, redirects, and normalisation. Remaining: M7.2 Help Center synchronisation, M7.3 native runtime search and per-Workspace provider selection, and M7.4 provider-backed knowledge connections. |
...
## 14. M7 — Knowledge and research intake

### Objective

Close the intake gaps the owner identified after M6 without adding a generic connector platform: richer knowledge sources, a synchronised Intercom Help Center, and a public-search registry with more than one approved provider.

### M7.1 Knowledge document formats

- Extend the existing knowledge upload and version contract to accept Markdown, HTML, and PDF documents and ZIP bundles of those files. Convert each to the retained plain-text snapshot the search index already uses; keep the original upload under the current attachment scan, size, and authorisation controls.
- Preserve one immutable Knowledge source version per file with digest, retrieval time, author, and expiry. A ZIP bundle creates one source per contained file and rejects paths that escape the bundle, nested archives, and unsupported types.
- Choose the smallest dependency that can extract PDF text and record it in `DEPENDENCIES.md` with the owner's approval before it enters the Gemfile. HTML and Markdown conversion use the Ruby standard library and the existing HTML sanitiser.
- Keep the extraction step inside Rails jobs with bounded bytes, pages, and time. Extracted text is untrusted evidence, never instruction.

### M7.2 Intercom Help Center synchronisation

- Replace manual Help Center snapshot registration with a read-only synchronisation through the existing Intercom connection, reconciliation cursor, and signed-webhook boundary. Use only GET requests against `https://api.intercom.io` articles and collections.
- Create or version one Knowledge source per published article, keep the remote article ID and updated time as the version identity, mark unpublished or deleted articles stale and then deleted, and never delete cited history.
- Bound each sync run by pages, bytes, and time; stop for review on an ambiguous or oversized article rather than truncating silently.

### M7.3 Public-search provider registry

- Add at least one hosted search adapter beside SearXNG behind the existing provider-neutral `web_search` contract in the Go runner. Prefer a provider whose API returns structured results with URL, title, excerpt, and publication date, needs only one API key held on the runner, and reports cost units. Exa and Tavily satisfy this and are implemented; SearXNG stays the packaged default for deployments that do not want a hosted provider.
- Offer a runtime's native search (for example Codex's built-in search) as a third option only when the run's egress profile permits it and the adapter can return auditable structured results with URLs and retrieval times; record the provider as the runtime, keep the same citation contract, and never let native search replace the runner's policy check.
- Keep the same normalisation, redaction, HTTPS-only evidence links, deduplication, and idempotency file. Allow a Workspace to select an approved provider from the runner catalog; a provider is unavailable until the deployment owner enables it in the execution policy.
- Record the chosen providers, their credential custody, and their data-handling terms in `DEPENDENCIES.md` and `THREAT_MODEL.md` before enabling them.

### M7.4 Provider-backed knowledge connections

- Allow a Workspace Owner or Admin to register an external knowledge connection (for example a Notion workspace export or API) through the same in-app connection flow and runner-held credential vault used for model providers. Start with one connection kind chosen from pilot evidence; do not add a generic connector SDK.
- Imported pages become Knowledge source versions with the same provenance, freshness, and deletion rules as uploads.

### Completion evidence

- Fixtures prove Markdown, HTML, PDF, and ZIP ingestion, path-escape and nested-archive rejection, size and page bounds, and stale or deleted handling.
- Help Center sync proves create, update, unpublish, delete, cursor resume, and cross-Workspace denial without any remote write.
- Each search provider passes the existing citation, redaction, and idempotency contract tests plus a provider fixture suite.
- Browser evidence covers upload, sync status, and provider selection at desktop and mobile widths with keyboard access.
...
| 6 September 2026 | In-app provider connections, the encrypted runner-local vault, and direct OpenAI and Anthropic API-key execution are an approved extension of the v1 runtime registry under interview decision Q12. They keep credentials on the runner and do not change the human-send model. |
| 6 September 2026 | The macOS host-trusted execution mode for Codex and Cursor is marked for redesign. A server-side companion boundary is the intended direction; the local desktop mode stays disabled by default and is recorded as an operator-accepted risk in the threat model until replaced. |
...
| 6 September 2026 | SearXNG-only public search, text-only knowledge uploads, and manual Intercom Help Center snapshots were gaps, not decisions. M7 covers knowledge and research intake. |
| 6 September 2026 | Ship a reference ClamAV attachment-scanner adapter and a daily scheduled Account-health pass instead of leaving both to operator-authored code and cron. |
| 6 September 2026 | Move the PostgreSQL pin from 15 to 16 (the newest major the check host and Compose tooling could exercise); the pgvector image is pinned at `0.8.6-pg16` by digest and OPERATIONS.md carries the major-version upgrade procedure. |
| 6 September 2026 | The landing page intentionally omits the template's pricing and testimonial sections until those exist; its hero shows a content-fitted product preview with cited evidence rather than a fixed 16:9 media stage. |
| 6 September 2026 | Approve the `pdf-reader` gem for knowledge PDF extraction. ZIP bundles use a bounded standard-library reader rather than an archive gem. |
