...
class GuardedWebFetcher
  ...
  def self.normalize_url(value)
    ...
    unless uri.is_a?(URI::HTTPS) && uri.host.present? && uri.userinfo.nil?
      invalid_source!("Use a public HTTPS URL without credentials.")
    end
    ...
    invalid_source!("Use a valid HTTPS URL.")
  end
  ...
  def fetch(url, now: Time.current)
    ...
    loop do
      ...
      if response.status.in?(300..399)
        invalid_source!("The source redirected without a location.") if response.location.blank?
        invalid_source!("The source redirected too many times.") if redirects >= MAX_REDIRECTS

        uri = parse_url(URI.join(uri.to_s, response.location).to_s)
        redirects += 1
        next
      end
      invalid_source!("The source returned HTTP #{response.status}.") unless response.status.in?(200..299)

      content_type = response.content_type.to_s.downcase.split(";", 2).first
      unless ALLOWED_CONTENT_TYPES.include?(content_type)
        invalid_source!("The source must return plain text or HTML.")
      end
      if response.content_length.to_i > MAX_BYTES || response.body.bytesize > MAX_BYTES
        invalid_source!("The fetched source must be 1 MiB or less.")
      end

      content = content_type == "text/plain" ? normalize_text(response.body) : html_to_text(response.body)
      invalid_source!("The fetched source has no readable text.") if content.blank?
    ...
    Rails.logger.warn("Guarded web fetch failed: #{error.class}")
    invalid_source!("The source could not be fetched securely.")
  end
  ...
  private
    ...
    def public_address_for(host)
      ...
      if addresses.empty? || addresses.any? { |address| BLOCKED_NETWORKS.any? { |network| network.include?(address) } }
        invalid_source!("The source URL must resolve only to public addresses.")
      end

      addresses.first.to_s
    rescue IPAddr::InvalidAddressError
      invalid_source!("The source URL has an invalid address.")
    end
    ...
    def request(uri, address)
      ...
      request = Net::HTTP::Get.new(uri.request_uri, {
        ...
        "User-Agent" => "NavishAI guarded web fetch"
      })
      ...
      http.start do |connection|
        connection.request(request) do |net_response|
          if net_response["content-encoding"].present? && net_response["content-encoding"] != "identity"
            invalid_source!("The source returned unsupported content encoding.")
          end
    ...
    private_class_method :blocked_address?, :local_hostname?, :invalid_source!
end
