class WorkspaceInvitationAcceptancesController < ApplicationController
  ...
  def create
    ...
    session[:return_to_after_authenticating] = workspace_invitation_acceptance_path(params[:token])
    ...
    redirect_to workspace_invitation_acceptance_path(params[:token]), alert: error.message
  end
