...
CI.run do
  ...
  step "Tests: Runner", "go vet ./... && NAVISHAI_REQUIRE_ISOLATION_TESTS=1 go test ./... && go build -o tmp/navishai-runner ./runner/cmd/navishai-runner && go build -o tmp/navishai-exec ./runner/cmd/navishai-exec && cc -std=c11 -O2 -Wall -Wextra -Werror -o tmp/navishai-netns-launch runner/cmd/navishai-netns-launch/main.c && cc -O2 -Wall -Wextra -Werror -o tmp/navishai-document runner/cmd/navishai-document/main.c -ldl"
