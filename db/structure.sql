...
CREATE TABLE public.operational_checks (
    ...
    CONSTRAINT operational_checks_kind CHECK (((check_kind)::text = ANY (ARRAY[('archive_verification'::character varying)::text, ('attachment_scanner'::character varying)::text, ('backup_verification'::character varying)::text, ('memory_verification'::character varying)::text, ('restore_rehearsal'::character varying)::text, ('upgrade_preflight'::character varying)::text]))),
    CONSTRAINT operational_checks_result CHECK (((result)::text = ANY (ARRAY[('passed'::character varying)::text, ('failed'::character varying)::text, ('unavailable'::character varying)::text, ('pending'::character varying)::text]))),
